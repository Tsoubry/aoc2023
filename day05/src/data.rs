...
pub struct Range {
    ...
    source_range_start: AnswerDtype,
    range_length: AnswerDtype,
}

pub type Parsed = (Vec<AnswerDtype>, Vec<Vec<Range>>);
...
pub fn import_data(data: &str) -> Parsed {
    ...
    let maps: Vec<_> = split_lines
        .map(|s| {
            ...
            lines.map(|line| parse(line)).collect::<Vec<Range>>()
        })
        .collect();
...
pub fn parse(line: &str) -> Range {
    ...
    Range {
        destination_range_start: *split_white.get(0).expect("get range item"),
        source_range_start: *split_white.get(1).expect("get range item"),
        range_length: *split_white.get(2).expect("get range item"),
    }
}
