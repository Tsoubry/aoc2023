...
[[ $# -eq 0 ]] && echo "Usage: ./no_std_generate.sh day"
...
cargo generate --path no-std-template --name no-std-day$DAY_FORMATTED

curl -f $URL/input -H "cookie: $(cat cookie)" > no-std-day$DAY_FORMATTED/input.txt 2> /dev/null
